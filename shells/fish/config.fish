...
mkdir -p "$DOTFILES_STATE_DIR" 2>/dev/null; or set -gx DOTFILES_STATE_DIR "/tmp/dotfiles-$USER"
mkdir -p "$DOTFILES_STATE_DIR" 2>/dev/null
test -w "$DOTFILES_STATE_DIR"; or set -gx DOTFILES_STATE_DIR "/tmp/dotfiles-$USER"
mkdir -p "$DOTFILES_STATE_DIR" 2>/dev/null
...
if test "$DOTFILES_LOAD_FULL" = "1"
    source "$DOTFILES_DIR/shells/fish/functions.fish"
    source "$DOTFILES_DIR/shells/fish/aliases.fish"
else
    source "$DOTFILES_DIR/shells/fish/functions-core.fish"
    source "$DOTFILES_DIR/shells/fish/aliases-core.fish"
end
...
if test "$DOTFILES_AUTO_UPDATE" = "1"; and test -f "$DOTFILES_DIR/bin/dotupdate_bg.sh"; and test "$DOTFILES_MODE" != "minimal"
...
set -q DOTFILES_FASTFETCH_ON_STARTUP; or set -gx DOTFILES_FASTFETCH_ON_STARTUP 1
if status is-interactive; and test "$DOTFILES_FASTFETCH_ON_STARTUP" = "1"; and type -q fastfetch
