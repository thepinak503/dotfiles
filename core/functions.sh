...
file_encrypt() { if [ $# -lt 1 ]; then echo "usage: FILE_PASS=secret file_encrypt <file>"; return 1; fi; openssl enc -aes-256-cbc -salt -in "$1" -out "${1}.enc" -k "${FILE_PASS:?}" 2>/dev/null && echo "encrypted: ${1}.enc"; }
file_decrypt() { if [ $# -lt 1 ]; then echo "usage: FILE_PASS=secret file_decrypt <file.enc>"; return 1; fi; local _out="${1%.enc}"; openssl enc -d -aes-256-cbc -in "$1" -out "$_out" -k "${FILE_PASS:?}" 2>/dev/null && echo "decrypted: $_out"; }
